...
namespace HieuBon
{
    public sealed class BotRadarView : MonoBehaviour
    {
            ...
            for (int i = 1; i < VisionConeResolution; i++)
            {
                ...
                if (Physics.Raycast(transform.position, dirs[i], out hit, VisionRange, layer))
                {
                    if (hit.collider != null)
                    {
                        ...
                        if (sentryBot != null && hit.collider.gameObject.name.Contains("mixamorig") && !hit.collider.attachedRigidbody.isKinematic && !allies.Contains(hit.collider.gameObject))
                        {
                            Bot bot = LevelController.instance.GetBoneNBot(allies, hit.collider.gameObject);
